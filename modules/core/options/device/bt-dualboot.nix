...
in {
  ...
  options.services.bt-dualboot = {
    ...
    package = lib.mkPackageOption self'.packages "bt-dualboot" {};
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.bt-dualboot =
      {
        ...
        serviceConfig = {
          ...
          ExecStop =
            "${cfg.package}/bin/bt-dualboot --sync-all"
            + (
              if cfg.registryBackups.enable
              then " --backup"
              else " --no-backup"
            )
    ...
    systemd.tmpfiles.settings.bt-dualboot."/var/backup/bt-dualboot/".e.age = let
